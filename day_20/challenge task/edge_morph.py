...
for image in images:
    ...
    if img is None:
        ...
        continue
    cv2.imwrite("challenge task/output/original_"+image,img)
    gray=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    blur=cv2.GaussianBlur(gray,(7,7),0)
    canny=cv2.Canny(blur,50,150)
    cv2.imwrite("challenge task/output/edge_"+image,canny)
    kernel=np.ones((7,7),np.uint8)
    closing=cv2.morphologyEx(canny,cv2.MORPH_CLOSE,kernel)
    closing=cv2.dilate(closing,kernel,iterations=1)
    closing=cv2.erode(closing,kernel,iterations=1)
    cv2.imwrite("Challenge Task/output/morph_"+image,closing)
    ...
    cv2.imwrite("challenge task/output/boundary_"+image,img)
